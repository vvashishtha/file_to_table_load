...
file_name='source_data_test.xlsx'#sys.argv[1]
sheets_name='Sheet1'#sys.argv[2]
Mapping_name='Mapping_test.xlsx'

df_excel = pd.read_excel(file_name, sheet_name=sheets_name)
df_excel1= pd.read_excel(file_name, sheet_name=sheets_name)
...
Sql_execute="insert into Demo_table ("
...
for row in c:
    print (row[0], "-", row[1])
print("Print finished, now insertion will start")

c.executemany(sql_demo, df_excel1, batcherrors = True)
